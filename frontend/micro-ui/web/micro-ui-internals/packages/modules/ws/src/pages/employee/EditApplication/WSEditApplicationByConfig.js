...
const WSEditApplicationByConfig = () => {
  ...
  const onFormValueChange = (setValue, formData, formState) => {
    ...
    if (
      ...
      formData.ConnectionDetails?.[0]?.water &&
      (formData.roadCuttingDetails?.[0]?.roadType?.code == "" ||
      ...
      formData.ConnectionDetails?.[0]?.sewerage &&
      (formData.roadCuttingDetails?.[0]?.roadType?.code == "" ||
        formData.roadCuttingDetails?.[0]?.area == "" ||
        formData.plumberDetails?.[0]?.detailsProvidedBy == "" ||
  ...
  const onSubmit = async (data) => {
    if (!canSubmit) {
      ...
      if (sessionAhocFormData?.adhocPenaltyComment)
        convertAppData.additionalDetails.adhocPenaltyComment = sessionAhocFormData?.adhocPenaltyComment || "";
      if (sessionAhocFormData?.adhocPenaltyReason)
        convertAppData.additionalDetails.adhocPenaltyReason = sessionAhocFormData?.adhocPenaltyReason || "";
      if (sessionAhocFormData?.adhocRebate) convertAppData.additionalDetails.adhocRebate = parseInt(sessionAhocFormData?.adhocRebate);
      if (sessionAhocFormData?.adhocRebateComment)
        convertAppData.additionalDetails.adhocRebateComment = sessionAhocFormData?.adhocRebateComment || "";
  ...
  return (
    <div className="employee-form-content-with-action-bar ">
      ...
      ></FormComposer>
      {showToast && <Toast error={showToast.key} label={t(showToast?.message)} warning={showToast?.warning} onClose={() => setShowToast(null)} />}
    </div>
  );
};
