...
const NewApplication = () => {
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      config?.body?.forEach((data) => {
        if (data?.isCreateConnection) bodyDetails.push(data);
      });
  ...
  const onFormValueChange = (setValue, formData, formState) => {
    ...
    if (
      ...
      Object.values(formState.errors["ConnectionHolderDetails"].type).filter((ob) => ob.type === "required" && ob?.ref?.value !== "").length >
        0 /*&&  !formData?.cpt?.details?.propertyId*/
    )
      setSubmitValve(true);
